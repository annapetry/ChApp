var chatServer = function (server) {
  ...
  var validNick = function (nick) {
    ...
    if (!!nick.match(/^mystery\d*$/i)) { // Working! yayayyyyyy!
      isValid = false;
    }
  ...
  io.on('connection', function (socket) {
    nicknames[socket.id] = 'Mystery' + guestnumber;
    ...
    io.to(currentRooms[socket.id]).emit('message', {
      nick: 'Bot',
      text: nicknames[socket.id] + ' has joined the ' + currentRooms[socket.id],
      addClass: 'bot'
    });
    ...
    socket.on('disconnect', function () {
      io.to(currentRooms[socket.id]).emit('message', {
        nick: 'Bot',
        text: nicknames[socket.id] + ' has left the ' + currentRooms[socket.id],
        addClass: 'bot'
      });
